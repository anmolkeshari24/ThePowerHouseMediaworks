...
function cross(t, e, i) {
    let r = e[0],
        ...
        h = i[1],
        o = i[2];
    return t[0] = s * o - n * h, t[1] = n * a - r * o, t[2] = r * h - s * a, t
}
...
function transformQuat(t, e, i) {
    let r = e[0],
        ...
        p = a * c - h * u,
        m = 2 * l;
    return u *= m, c *= m, _ *= m, d *= 2, g *= 2, p *= 2, t[0] = r + u + d, t[1] = s + c + g, t[2] = n + _ + p, t
}
let angle = function() {
    let t = [0, 0, 0],
        e = [0, 0, 0];
    return function(i, r) {
        copy(t, i), copy(e, r), normalize(t, t), normalize(e, e);
        let s = dot(t, e);
...
class Vec3 extends Array {
    ...
    set(t, e = t, i = t) {
        return t.length ? this.copy(t) : (set(this, t, e, i), this)
    }
    copy(t) {
        return copy(this, t), this
    }
    add(t, e) {
        return e ? add(this, t, e) : add(this, this, t), this
    }
    sub(t, e) {
        return e ? subtract(this, t, e) : subtract(this, this, t), this
    }
    multiply(t) {
        return t.length ? multiply(this, this, t) : scale(this, this, t), this
    }
    divide(t) {
        return t.length ? divide(this, this, t) : scale(this, this, 1 / t), this
    }
    inverse(t = this) {
        return inverse(this, t), this
    }
    len() {
        return length(this)
    }
    ...
    negate(t = this) {
        return negate(this, t), this
    }
    cross(t, e) {
        return cross(this, t, e), this
    }
    scale(t) {
        return scale(this, this, t), this
    }
    normalize() {
        return normalize(this, this), this
    }
    dot(t) {
        return dot(this, t)
    }
    ...
    applyMatrix4(t) {
        return transformMat4(this, this, t), this
    }
    applyQuaternion(t) {
        return transformQuat(this, this, t), this
    }
    angle(t) {
        return angle(this, t)
    }
    lerp(t, e) {
        return lerp(this, this, t, e), this
    }
    clone() {
...
class Geometry {
    ...
    computeBoundingBox(t) {
        ...
        let e = this.bounds.min,
            ...
            s = this.bounds.scale;
        e.set(Infinity), i.set(-1 / 0);
        for (let n = 0, a = t.length; n < a; n += 3) {
            let h = t[n],
                o = t[n + 1],
                l = t[n + 2];
            e.x = Math.min(h, e.x), e.y = Math.min(o, e.y), e.z = Math.min(l, e.z), i.x = Math.max(h, i.x), i.y = Math.max(o, i.y), i.z = Math.max(l, i.z)
        }
    ...
    computeBoundingSphere(t) {
        ...
        let e = 0;
        for (let i = 0, r = t.length; i < r; i += 3) tempVec3.fromArray(t, i), e = Math.max(e, this.bounds.center.squaredDistance(tempVec3));
        this.bounds.radius = Math.sqrt(e)
    }
    remove() {
        for (let t in this.vao && this.gl.renderer.deleteVertexArray(this.vao), this.attributes) this.gl.deleteBuffer(this.attributes[t].buffer), delete this.attributes[t]
    }
}
let ID$1 = 0,
    arrayCacheF32 = {};
...
Fragment Shader
${addLineNumbers(i)}`), this.program = t.createProgram(), t.attachShader(this.program, u), t.attachShader(this.program, c), t.linkProgram(this.program), !t.getProgramParameter(this.program, t.LINK_STATUS)) return console.warn(t.getProgramInfoLog(this.program));
        ...
        for (let d = 0; d < _; d++) {
            ...
            g.uniformName = p[0], 3 === p.length ? (g.isStructArray = !0, g.structIndex = Number(p[1]), g.structProperty = p[2]) : 2 === p.length && isNaN(Number(p[1])) && (g.isStruct = !0, g.structProperty = p[1])
        }
        ...
        for (let f = 0; f < $; f++) {
            ...
            m[v] = x.name, this.attributeLocations.set(x.name, v)
        }
    ...
    use({
        ...
        i || (this.gl.useProgram(this.program), this.gl.renderer.currentProgram = this.id), this.uniformLocations.forEach((t, i) => {
            let r = i.uniformName,
                s = this.uniforms[r];
            if (i.isStruct && (s = s[i.structProperty], r += `.${i.structProperty}`), i.isStructArray && (s = s[i.structIndex][i.structProperty], r += `[${i.structIndex}].${i.structProperty}`), !s) return warn(`Active uniform ${r} has not been supplied`);
            if (s && void 0 === s.value) return warn(`${r} uniform is missing a value parameter`);
            if (s.value.texture) return e += 1, s.value.update(e), setUniform(this.gl, i.type, t, e);
...
function setUniform(t, e, i, r) {
    ...
    if (r.length) {
        if (void 0 === s) t.renderer.state.uniformLocations.set(i, r.slice(0));
            ...
            if (arraysEqual(s, r)) return;
            s.set ? s.set(r) : setArray(s, r), t.renderer.state.uniformLocations.set(i, s)
        }
    } else {
        if (s === r) return;
...
function flatten(t) {
    ...
    for (let n = 0; n < e; n++) s.set(t[n], n * i);
    return s
}
...
function warn(t) {
    !(warnCount > 100) && (console.warn(t), ++warnCount > 100 && console.warn("More than 100 program warnings - stopping logs."))
}
...
class Renderer {
    ...
    getRenderList({
        ...
        if (e && i && e.updateFrustum(), t.traverse(t => {
                ...
                if (t.draw) {
                    if (i && t.frustumCulled && e && !e.frustumIntersectsMesh(t)) return;
                    s.push(t)
                }
            }), r) {
            let n = [],
                ...
                h = [];
            s.forEach(t => {
                t.program.transparent ? t.program.depthTest ? a.push(t) : h.push(t) : n.push(t), t.zDepth = 0, 0 === t.renderOrder && t.program.depthTest && e && (t.worldMatrix.getTranslation(tempVec3$1), tempVec3$1.applyMatrix4(e.projectionViewMatrix), t.zDepth = tempVec3$1.z)
            }), n.sort(this.sortOpaque), a.sort(this.sortTransparent), h.sort(this.sortUI), s = n.concat(a, h)
        }
        return s
    }
    render({
...
function rotateX(t, e, i) {
    ...
    let r = e[0],
        ...
        h = Math.sin(i),
        o = Math.cos(i);
    return t[0] = r * o + a * h, t[1] = s * o + n * h, t[2] = n * o - s * h, t[3] = a * o - r * h, t
}
...
function rotateY(t, e, i) {
    ...
    let r = e[0],
        ...
        h = Math.sin(i),
        o = Math.cos(i);
    return t[0] = r * o - n * h, t[1] = s * o + a * h, t[2] = n * o + r * h, t[3] = a * o - s * h, t
}
...
function rotateZ(t, e, i) {
    ...
    let r = e[0],
        ...
        h = Math.sin(i),
        o = Math.cos(i);
    return t[0] = r * o + s * h, t[1] = s * o - r * h, t[2] = n * o + a * h, t[3] = a * o - n * h, t
}
...
function slerp(t, e, i, r) {
    let s = e[0],
        ...
        c = i[3],
        _, d, g, p, m;
    return (d = s * o + n * l + a * u + h * c) < 0 && (d = -d, o = -o, l = -l, u = -u, c = -c), 1 - d > 1e-6 ? (p = Math.sin((1 - r) * _) / (g = Math.sin(_ = Math.acos(d))), m = Math.sin(r * _) / g) : (p = 1 - r, m = r), t[0] = p * s + m * o, t[1] = p * n + m * l, t[2] = p * a + m * u, t[3] = p * h + m * c, t
}
...
function invert(t, e) {
    let i = e[0],
        ...
        a = i * i + r * r + s * s + n * n,
        h = a ? 1 / a : 0;
    return t[0] = -i * h, t[1] = -r * h, t[2] = -s * h, t[3] = n * h, t
}
...
function fromMat3(t, e) {
    ...
    if (i > 0) r = Math.sqrt(i + 1), t[3] = .5 * r, r = .5 / r, t[0] = (e[5] - e[7]) * r, t[1] = (e[6] - e[2]) * r, t[2] = (e[1] - e[3]) * r;
        ...
        let n = (s + 1) % 3,
            a = (s + 2) % 3;
        r = Math.sqrt(e[3 * s + s] - e[3 * n + n] - e[3 * a + a] + 1), t[s] = .5 * r, r = .5 / r, t[3] = (e[3 * n + a] - e[3 * a + n]) * r, t[n] = (e[3 * n + s] + e[3 * s + n]) * r, t[a] = (e[3 * a + s] + e[3 * s + a]) * r
    }
    return t
}
...
function fromEuler(t, e, i = "YXZ") {
    let r = Math.sin(.5 * e[0]),
        ...
        h = Math.sin(.5 * e[2]),
        o = Math.cos(.5 * e[2]);
    return "XYZ" === i ? (t[0] = r * a * o + s * n * h, t[1] = s * n * o - r * a * h, t[2] = s * a * h + r * n * o, t[3] = s * a * o - r * n * h) : "YXZ" === i ? (t[0] = r * a * o + s * n * h, t[1] = s * n * o - r * a * h, t[2] = s * a * h - r * n * o, t[3] = s * a * o + r * n * h) : "ZXY" === i ? (t[0] = r * a * o - s * n * h, t[1] = s * n * o + r * a * h, t[2] = s * a * h + r * n * o, t[3] = s * a * o - r * n * h) : "ZYX" === i ? (t[0] = r * a * o - s * n * h, t[1] = s * n * o + r * a * h, t[2] = s * a * h - r * n * o, t[3] = s * a * o + r * n * h) : "YZX" === i ? (t[0] = r * a * o + s * n * h, t[1] = s * n * o + r * a * h, t[2] = s * a * h - r * n * o, t[3] = s * a * o - r * n * h) : "XZY" === i && (t[0] = r * a * o - s * n * h, t[1] = s * n * o - r * a * h, t[2] = s * a * h + r * n * o, t[3] = s * a * o + r * n * h), t
}
let copy$2 = copy$1,
    set$2 = set$1,
    dot$2 = dot$1,
...
class Quat extends Array {
    ...
    identity() {
        return identity(this), this.onChange(), this
    }
    set(t, e, i, r) {
        return t.length ? this.copy(t) : (set$2(this, t, e, i, r), this.onChange(), this)
    }
    rotateX(t) {
        return rotateX(this, this, t), this.onChange(), this
    }
    rotateY(t) {
        return rotateY(this, this, t), this.onChange(), this
    }
    rotateZ(t) {
        return rotateZ(this, this, t), this.onChange(), this
    }
    inverse(t = this) {
        return invert(this, t), this.onChange(), this
    }
    conjugate(t = this) {
        return conjugate(this, t), this.onChange(), this
    }
    copy(t) {
        return copy$2(this, t), this.onChange(), this
    }
    normalize(t = this) {
        return normalize$2(this, t), this.onChange(), this
    }
    multiply(t, e) {
        return e ? multiply$1(this, t, e) : multiply$1(this, this, t), this.onChange(), this
    }
    dot(t) {
        return dot$2(this, t)
    }
    fromMatrix3(t) {
        return fromMat3(this, t), this.onChange(), this
    }
    fromEuler(t) {
        return fromEuler(this, t, t.order), this
    }
    fromAxisAngle(t, e) {
        return setAxisAngle(this, t, e), this
    }
    slerp(t, e) {
        return slerp(this, this, t, e), this
    }
    fromArray(t, e = 0) {
...
function set$3(t, e, i, r, s, n, a, h, o, l, u, c, _, d, g, p, m) {
    return t[0] = e, t[1] = i, t[2] = r, t[3] = s, t[4] = n, t[5] = a, t[6] = h, t[7] = o, t[8] = l, t[9] = u, t[10] = c, t[11] = _, t[12] = d, t[13] = g, t[14] = p, t[15] = m, t
}
...
function translate(t, e, i) {
    let r = i[0],
        s = i[1],
        n = i[2],
        a, h, o, l, u, c, _, d, g, p, m, $;
    return e === t ? (t[12] = e[0] * r + e[4] * s + e[8] * n + e[12], t[13] = e[1] * r + e[5] * s + e[9] * n + e[13], t[14] = e[2] * r + e[6] * s + e[10] * n + e[14], t[15] = e[3] * r + e[7] * s + e[11] * n + e[15]) : (a = e[0], h = e[1], o = e[2], l = e[3], u = e[4], c = e[5], _ = e[6], d = e[7], g = e[8], p = e[9], m = e[10], $ = e[11], t[0] = a, t[1] = h, t[2] = o, t[3] = l, t[4] = u, t[5] = c, t[6] = _, t[7] = d, t[8] = g, t[9] = p, t[10] = m, t[11] = $, t[12] = a * r + u * s + g * n + e[12], t[13] = h * r + c * s + p * n + e[13], t[14] = o * r + _ * s + m * n + e[14], t[15] = l * r + d * s + $ * n + e[15]), t
}
...
function scale$1(t, e, i) {
    let r = i[0],
        s = i[1],
        n = i[2];
    return t[0] = e[0] * r, t[1] = e[1] * r, t[2] = e[2] * r, t[3] = e[3] * r, t[4] = e[4] * s, t[5] = e[5] * s, t[6] = e[6] * s, t[7] = e[7] * s, t[8] = e[8] * n, t[9] = e[9] * n, t[10] = e[10] * n, t[11] = e[11] * n, t[12] = e[12], t[13] = e[13], t[14] = e[14], t[15] = e[15], t
}
...
function rotateX$1(t, e, i) {
    let r = Math.sin(i),
        ...
        c = e[10],
        _ = e[11];
    return e !== t && (t[0] = e[0], t[1] = e[1], t[2] = e[2], t[3] = e[3], t[12] = e[12], t[13] = e[13], t[14] = e[14], t[15] = e[15]), t[4] = n * s + l * r, t[5] = a * s + u * r, t[6] = h * s + c * r, t[7] = o * s + _ * r, t[8] = l * s - n * r, t[9] = u * s - a * r, t[10] = c * s - h * r, t[11] = _ * s - o * r, t
}
...
function rotateY$1(t, e, i) {
    let r = Math.sin(i),
        ...
        c = e[10],
        _ = e[11];
    return e !== t && (t[4] = e[4], t[5] = e[5], t[6] = e[6], t[7] = e[7], t[12] = e[12], t[13] = e[13], t[14] = e[14], t[15] = e[15]), t[0] = n * s - l * r, t[1] = a * s - u * r, t[2] = h * s - c * r, t[3] = o * s - _ * r, t[8] = n * r + l * s, t[9] = a * r + u * s, t[10] = h * r + c * s, t[11] = o * r + _ * s, t
}
...
function rotateZ$1(t, e, i) {
    let r = Math.sin(i),
        ...
        c = e[6],
        _ = e[7];
    return e !== t && (t[8] = e[8], t[9] = e[9], t[10] = e[10], t[11] = e[11], t[12] = e[12], t[13] = e[13], t[14] = e[14], t[15] = e[15]), t[0] = n * s + l * r, t[1] = a * s + u * r, t[2] = h * s + c * r, t[3] = o * s + _ * r, t[4] = l * s - n * r, t[5] = u * s - a * r, t[6] = c * s - h * r, t[7] = _ * s - o * r, t
}
...
function getScaling(t, e) {
    let i = e[0],
        ...
        l = e[9],
        u = e[10];
    return t[0] = Math.sqrt(i * i + r * r + s * s), t[1] = Math.sqrt(n * n + a * a + h * h), t[2] = Math.sqrt(o * o + l * l + u * u), t
}
...
function fromRotationTranslationScale(t, e, i, r) {
    let s = e[0],
        ...
        b = r[1],
        w = r[2];
    return t[0] = (1 - (g + m)) * v, t[1] = (_ + x) * v, t[2] = (d - f) * v, t[3] = 0, t[4] = (_ - x) * b, t[5] = (1 - (c + m)) * b, t[6] = (p + $) * b, t[7] = 0, t[8] = (d + f) * w, t[9] = (p - $) * w, t[10] = (1 - (c + g)) * w, t[11] = 0, t[12] = i[0], t[13] = i[1], t[14] = i[2], t[15] = 1, t
}
...
function fromQuat(t, e) {
    let i = e[0],
        ...
        m = n * h,
        $ = n * o;
    return t[0] = 1 - c - g, t[1] = u + $, t[2] = _ - m, t[3] = 0, t[4] = u - $, t[5] = 1 - l - g, t[6] = d + p, t[7] = 0, t[8] = _ + m, t[9] = d - p, t[10] = 1 - l - c, t[11] = 0, t[12] = 0, t[13] = 0, t[14] = 0, t[15] = 1, t
}

function perspective(t, e, i, r, s) {
    let n = 1 / Math.tan(e / 2),
        a = 1 / (r - s);
    return t[0] = n / i, t[1] = 0, t[2] = 0, t[3] = 0, t[4] = 0, t[5] = n, t[6] = 0, t[7] = 0, t[8] = 0, t[9] = 0, t[10] = (s + r) * a, t[11] = -1, t[12] = 0, t[13] = 0, t[14] = 2 * s * r * a, t[15] = 0, t
}
...
function ortho(t, e, i, r, s, n, a) {
    let h = 1 / (e - i),
        o = 1 / (r - s),
        l = 1 / (n - a);
    return t[0] = -2 * h, t[1] = 0, t[2] = 0, t[3] = 0, t[4] = 0, t[5] = -2 * o, t[6] = 0, t[7] = 0, t[8] = 0, t[9] = 0, t[10] = 2 * l, t[11] = 0, t[12] = (e + i) * h, t[13] = (s + r) * o, t[14] = (a + n) * l, t[15] = 1, t
}
...
function targetTo(t, e, i, r) {
    let s = e[0],
        ...
        _ = a - i[2],
        d = u * u + c * c + _ * _;
    d > 0 && (u *= d = 1 / Math.sqrt(d), c *= d, _ *= d);
    let g = o * _ - l * c,
        p = l * u - h * _,
        m = h * c - o * u;
    return (d = g * g + p * p + m * m) > 0 && (g *= d = 1 / Math.sqrt(d), p *= d, m *= d), t[0] = g, t[1] = p, t[2] = m, t[3] = 0, t[4] = c * m - _ * p, t[5] = _ * g - u * m, t[6] = u * p - c * g, t[7] = 0, t[8] = u, t[9] = c, t[10] = _, t[11] = 0, t[12] = s, t[13] = n, t[14] = a, t[15] = 1, t
}
class Mat4 extends Array {
    constructor(t = 1, e = 0, i = 0, r = 0, s = 0, n = 1, a = 0, h = 0, o = 0, l = 0, u = 1, c = 0, _ = 0, d = 0, g = 0, p = 1) {
        return super(t, e, i, r, s, n, a, h, o, l, u, c, _, d, g, p), this
    }
    set x(t) {
        this[12] = t
    }
    ...
    set(t, e, i, r, s, n, a, h, o, l, u, c, _, d, g, p) {
        return t.length ? this.copy(t) : (set$3(this, t, e, i, r, s, n, a, h, o, l, u, c, _, d, g, p), this)
    }
    translate(t, e = this) {
        return translate(this, e, t), this
    }
    rotateX(t, e = this) {
        return rotateX$1(this, e, t), this
    }
    rotateY(t, e = this) {
        return rotateY$1(this, e, t), this
    }
    rotateZ(t, e = this) {
        return rotateZ$1(this, e, t), this
    }
    scale(t, e = this) {
        return scale$1(this, e, "number" == typeof t ? [t, t, t] : t), this
    }
    multiply(t, e) {
        return e ? multiply$2(this, t, e) : multiply$2(this, this, t), this
    }
    identity() {
        return identity$1(this), this
    }
    copy(t) {
        return copy$3(this, t), this
    }
    fromPerspective({
        fov: t,
        aspect: e,
        ...
        return perspective(this, t, e, i, r), this
    }
    fromOrthogonal({
        left: t,
        right: e,
        ...
        return ortho(this, t, e, i, r, s, n), this
    }
    fromQuaternion(t) {
        return fromQuat(this, t), this
    }
    setPosition(t) {
        return this.x = t[0], this.y = t[1], this.z = t[2], this
    }
    inverse(t = this) {
        return invert$1(this, t), this
    }
    compose(t, e, i) {
        return fromRotationTranslationScale(this, t, e, i), this
    }
    getRotation(t) {
        return getRotation(t, this), this
    }
    getTranslation(t) {
        return getTranslation(t, this), this
    }
    getScaling(t) {
        return getScaling(t, this), this
    }
    getMaxScaleOnAxis() {
        return getMaxScaleOnAxis(this)
    }
    lookAt(t, e, i) {
        return targetTo(this, t, e, i), this
    }
    determinant() {
...
class Euler extends Array {
    ...
    fromRotationMatrix(t, e = this.order) {
        return fromRotationMatrix(this, t, e), this
    }
    fromQuaternion(t, e = this.order) {
        return tmpMat4.fromQuaternion(t), this.fromRotationMatrix(tmpMat4, e)
    }
}
class Transform {
    constructor() {
        this.parent = null, this.children = [], this.visible = !0, this.matrix = new Mat4, this.worldMatrix = new Mat4, this.matrixAutoUpdate = !0, this.position = new Vec3, this.quaternion = new Quat, this.scale = new Vec3(1), this.rotation = new Euler, this.up = new Vec3(0, 1, 0), this.rotation.onChange = () => this.quaternion.fromEuler(this.rotation), this.quaternion.onChange = () => this.rotation.fromQuaternion(this.quaternion)
    }
    setParent(t, e = !0) {
...
class Camera extends Transform {
    constructor(t, {
        ...
        super(t), this.near = e, this.far = i, this.fov = r, this.aspect = s, this.projectionMatrix = new Mat4, this.viewMatrix = new Mat4, this.projectionViewMatrix = new Mat4, n || a ? this.orthographic({
            left: n,
            right: a,
            bottom: h,
            top: o
    ...
    unproject(t) {
        return t.applyMatrix4(tempMat4.inverse(this.projectionMatrix)), t.applyMatrix4(this.worldMatrix), this
    }
    updateFrustum() {
...
function set$4(t, e, i, r, s, n, a, h, o, l) {
    return t[0] = e, t[1] = i, t[2] = r, t[3] = s, t[4] = n, t[5] = a, t[6] = h, t[7] = o, t[8] = l, t
}
...
function invert$2(t, e) {
    let i = e[0],
        ...
        d = l * n - a * o,
        g = i * c + r * _ + s * d;
    return g ? (g = 1 / g, t[0] = c * g, t[1] = (-u * r + s * l) * g, t[2] = (h * r - s * a) * g, t[3] = _ * g, t[4] = (u * i - s * o) * g, t[5] = (-h * i + s * n) * g, t[6] = d * g, t[7] = (-l * i + r * o) * g, t[8] = (a * i - r * n) * g, t) : null
}
...
function multiply$3(t, e, i) {
    let r = e[0],
        ...
        x = i[7],
        v = i[8];
    return t[0] = _ * r + d * a + g * l, t[1] = _ * s + d * h + g * u, t[2] = _ * n + d * o + g * c, t[3] = p * r + m * a + $ * l, t[4] = p * s + m * h + $ * u, t[5] = p * n + m * o + $ * c, t[6] = f * r + x * a + v * l, t[7] = f * s + x * h + v * u, t[8] = f * n + x * o + v * c, t
}
...
function translate$1(t, e, i) {
    let r = e[0],
        ...
        _ = i[0],
        d = i[1];
    return t[0] = r, t[1] = s, t[2] = n, t[3] = a, t[4] = h, t[5] = o, t[6] = _ * r + d * a + l, t[7] = _ * s + d * h + u, t[8] = _ * n + d * o + c, t
}
...
function rotate(t, e, i) {
    let r = e[0],
        ...
        _ = Math.sin(i),
        d = Math.cos(i);
    return t[0] = d * r + _ * a, t[1] = d * s + _ * h, t[2] = d * n + _ * o, t[3] = d * a - _ * r, t[4] = d * h - _ * s, t[5] = d * o - _ * n, t[6] = l, t[7] = u, t[8] = c, t
}

function scale$2(t, e, i) {
    let r = i[0],
        s = i[1];
    return t[0] = r * e[0], t[1] = r * e[1], t[2] = r * e[2], t[3] = s * e[3], t[4] = s * e[4], t[5] = s * e[5], t[6] = e[6], t[7] = e[7], t[8] = e[8], t
}
...
function fromQuat$1(t, e) {
    let i = e[0],
        ...
        m = n * h,
        $ = n * o;
    return t[0] = 1 - c - g, t[3] = u - $, t[6] = _ + m, t[1] = u + $, t[4] = 1 - l - g, t[7] = d - p, t[2] = _ - m, t[5] = d + p, t[8] = 1 - l - c, t
}
...
function normalFromMat4(t, e) {
    let i = e[0],
        ...
        F = _ * $ - d * m,
        S = f * F - x * R + v * M + b * T - w * A + E * y;
    return S ? (S = 1 / S, t[0] = (h * F - o * R + l * M) * S, t[1] = (o * T - a * F - l * A) * S, t[2] = (a * R - h * T + l * y) * S, t[3] = (s * R - r * F - n * M) * S, t[4] = (i * F - s * T + n * A) * S, t[5] = (r * T - i * R - n * y) * S, t[6] = (p * E - m * w + $ * b) * S, t[7] = (m * v - g * E - $ * x) * S, t[8] = (g * w - p * v + $ * f) * S, t) : null
}
class Mat3 extends Array {
    ...
    set(t, e, i, r, s, n, a, h, o) {
        return t.length ? this.copy(t) : (set$4(this, t, e, i, r, s, n, a, h, o), this)
    }
    translate(t, e = this) {
        return translate$1(this, e, t), this
    }
    rotate(t, e = this) {
        return rotate(this, e, t), this
    }
    scale(t, e = this) {
        return scale$2(this, e, t), this
    }
    multiply(t, e) {
        return e ? multiply$3(this, t, e) : multiply$3(this, this, t), this
    }
    identity() {
        return identity$2(this), this
    }
    copy(t) {
        return copy$4(this, t), this
    }
    fromMatrix4(t) {
        return fromMat4(this, t), this
    }
    fromQuaternion(t) {
        return fromQuat$1(this, t), this
    }
    fromBasis(t, e, i) {
        return this.set(t[0], t[1], t[2], e[0], e[1], e[2], i[0], i[1], i[2]), this
    }
    inverse(t = this) {
        return invert$2(this, t), this
    }
    getNormalMatrix(t) {
        return normalFromMat4(this, t), this
    }
}
let ID$2 = 0;
class Mesh extends Transform {
    constructor(t, {
        ...
        super(t), this.gl = t, this.id = ID$2++, this.geometry = e, this.program = i, this.mode = r, this.frustumCulled = s, this.renderOrder = n, this.modelViewMatrix = new Mat4, this.normalMatrix = new Mat3, this.beforeRenderCallbacks = [], this.afterRenderCallbacks = []
    }
    onBeforeRender(t) {
...
class Texture {
    constructor(t, {
        ...
        unpackAlignment: _ = 4,
        flipY: d = i == t.TEXTURE_2D,
        level: g = 0,
        width: p,
        height: m = p
    } = {}) {
        this.gl = t, this.id = ID$3++, this.image = e, this.target = i, this.type = r, this.format = s, this.internalFormat = n, this.minFilter = l, this.magFilter = u, this.wrapS = a, this.wrapT = h, this.generateMipmaps = o, this.premultiplyAlpha = c, this.unpackAlignment = _, this.flipY = d, this.level = g, this.width = p, this.height = m, this.texture = this.gl.createTexture(), this.store = {
            image: null
    ...
    update(t = 0) {
        ...
        if ((e || this.glState.textureUnits[t] !== this.id) && (this.gl.renderer.activeTexture(t), this.bind()), e) {
            if (this.needsUpdate = !1, this.flipY !== this.glState.flipY && (this.gl.pixelStorei(this.gl.UNPACK_FLIP_Y_WEBGL, this.flipY), this.glState.flipY = this.flipY), this.premultiplyAlpha !== this.glState.premultiplyAlpha && (this.gl.pixelStorei(this.gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL, this.premultiplyAlpha), this.glState.premultiplyAlpha = this.premultiplyAlpha), this.unpackAlignment !== this.glState.unpackAlignment && (this.gl.pixelStorei(this.gl.UNPACK_ALIGNMENT, this.unpackAlignment), this.glState.unpackAlignment = this.unpackAlignment), this.minFilter !== this.state.minFilter && (this.gl.texParameteri(this.target, this.gl.TEXTURE_MIN_FILTER, this.minFilter), this.state.minFilter = this.minFilter), this.magFilter !== this.state.magFilter && (this.gl.texParameteri(this.target, this.gl.TEXTURE_MAG_FILTER, this.magFilter), this.state.magFilter = this.magFilter), this.wrapS !== this.state.wrapS && (this.gl.texParameteri(this.target, this.gl.TEXTURE_WRAP_S, this.wrapS), this.state.wrapS = this.wrapS), this.wrapT !== this.state.wrapT && (this.gl.texParameteri(this.target, this.gl.TEXTURE_WRAP_T, this.wrapT), this.state.wrapT = this.wrapT), this.image) {
                ...
                else ArrayBuffer.isView(this.image) ? this.gl.texImage2D(this.target, this.level, this.internalFormat, this.width, this.height, 0, this.format, this.type, this.image) : this.gl.texImage2D(this.target, this.level, this.internalFormat, this.format, this.type, this.image);
                this.generateMipmaps && (this.gl.renderer.isWebgl2 || isPowerOf2(this.image.width) && isPowerOf2(this.image.height) ? this.gl.generateMipmap(this.target) : (this.generateMipmaps = !1, this.wrapS = this.wrapT = this.gl.CLAMP_TO_EDGE, this.minFilter = this.gl.LINEAR))
            } else if (this.target === this.gl.TEXTURE_CUBE_MAP)
                for (let r = 0; r < 6; r++) this.gl.texImage2D(this.gl.TEXTURE_CUBE_MAP_POSITIVE_X + r, 0, this.gl.RGBA, 1, 1, 0, this.gl.RGBA, this.gl.UNSIGNED_BYTE, emptyPixel);
            else this.width ? this.gl.texImage2D(this.target, this.level, this.internalFormat, this.width, this.height, 0, this.format, this.type, null) : this.gl.texImage2D(this.target, 0, this.gl.RGBA, 1, 1, 0, this.gl.RGBA, this.gl.UNSIGNED_BYTE, emptyPixel);
            this.store.image = this.image, this.onUpdate && this.onUpdate()
        }
    }
}
class RenderTarget {
    constructor(t, {
        ...
        magFilter: c = u,
        type: _ = t.UNSIGNED_BYTE,
        format: d = t.RGBA,
        internalFormat: g = d,
        unpackAlignment: p,
        premultiplyAlpha: m
    } = {}) {
        this.gl = t, this.width = e, this.height = i, this.depth = n, this.buffer = this.gl.createFramebuffer(), this.target = r, this.gl.bindFramebuffer(this.target, this.buffer), this.textures = [];
        let $ = [];
        for (let f = 0; f < s; f++) this.textures.push(new Texture(t, {
            width: e,
            height: i,
            wrapS: o,
            wrapT: l,
            ...
            flipY: !1,
            generateMipmaps: !1
        })), this.textures[f].update(), this.gl.framebufferTexture2D(this.target, this.gl.COLOR_ATTACHMENT0 + f, this.gl.TEXTURE_2D, this.textures[f].texture, 0), $.push(this.gl.COLOR_ATTACHMENT0 + f);
        $.length > 1 && this.gl.renderer.drawBuffers($), this.texture = this.textures[0], h && (this.gl.renderer.isWebgl2 || this.gl.renderer.getExtension("WEBGL_depth_texture")) ? (this.depthTexture = new Texture(t, {
            width: e,
            height: i,
...
class Color extends Array {
    constructor(t = 0, e = 0, i = 0) {
        return "string" == typeof t && ([t, e, i] = hexToRGB(t)), super(t, e, i), this
    }
    get r() {
        return this[0]
    }
    ...
    set(t, e, i) {
        return ("string" == typeof t && ([t, e, i] = hexToRGB(t)), t.length) ? this.copy(t) : (this[0] = t, this[1] = e, this[2] = i, this)
    }
    copy(t) {
...
function lerp$1(t, e, i, r) {
    var s = e[0],
        n = e[1];
    return t[0] = s + r * (i[0] - s), t[1] = n + r * (i[1] - n), t
}

function transformMat3(t, e, i) {
    var r = e[0],
        s = e[1];
    return t[0] = i[0] * r + i[3] * s + i[6], t[1] = i[1] * r + i[4] * s + i[7], t
}

function transformMat4$1(t, e, i) {
    let r = e[0],
        s = e[1];
    return t[0] = i[0] * r + i[4] * s + i[12], t[1] = i[1] * r + i[5] * s + i[13], t
}
...
class Vec2 extends Array {
    ...
    set(t, e = t) {
        return t.length ? this.copy(t) : (set$5(this, t, e), this)
    }
    copy(t) {
        return copy$5(this, t), this
    }
    add(t, e) {
        return e ? add$1(this, t, e) : add$1(this, this, t), this
    }
    sub(t, e) {
        return e ? subtract$1(this, t, e) : subtract$1(this, this, t), this
    }
    multiply(t) {
        return t.length ? multiply$4(this, this, t) : scale$3(this, this, t), this
    }
    divide(t) {
        return t.length ? divide$1(this, this, t) : scale$3(this, this, 1 / t), this
    }
    inverse(t = this) {
        return inverse$1(this, t), this
    }
    len() {
    ...
    negate(t = this) {
        return negate$1(this, t), this
    }
    cross(t, e) {
        return cross$1(t, e)
    }
    scale(t) {
        return scale$3(this, this, t), this
    }
    normalize() {
        return normalize$3(this, this), this
    }
    dot(t) {
        return dot$3(this, t)
    }
    ...
    applyMatrix3(t) {
        return transformMat3(this, this, t), this
    }
    applyMatrix4(t) {
        return transformMat4$1(this, this, t), this
    }
    lerp(t, e) {
...
class Vec4 extends Array {
    ...
    set(t, e, i, r) {
        return t.length ? this.copy(t) : (set$1(this, t, e, i, r), this)
    }
    copy(t) {
        return copy$1(this, t), this
    }
    normalize() {
        return normalize$1(this, this), this
    }
    fromArray(t, e = 0) {
        return this[0] = t[e], this[1] = t[e + 1], this[2] = t[e + 2], this[3] = t[e + 3], this
    }
...
class Plane extends Geometry {
    constructor(t, {
        ...
        Plane.buildPlane(u, c, _, d, e, i, 0, a, h), Object.assign(n, {
            position: {
                size: 3,
                data: u
            },
    ...
    static buildPlane(t, e, i, r, s, n, a, h, o, l = 0, u = 1, c = 2, _ = 1, d = -1, g = 0, p = 0) {
        ...
        for (let x = 0; x <= o; x++) {
            ...
            for (let b = 0; b <= h; b++, g++) {
                let w = b * $ - s / 2;
                if (t[3 * g + l] = w * _, t[3 * g + u] = v * d, t[3 * g + c] = a / 2, e[3 * g + l] = 0, e[3 * g + u] = 0, e[3 * g + c] = a >= 0 ? 1 : -1, i[2 * g] = b / h, i[2 * g + 1] = 1 - x / o, x === o || b === h) continue;
                let E = m + b + x * (h + 1),
                    y = m + b + (x + 1) * (h + 1),
                    A = m + b + (x + 1) * (h + 1) + 1,
                    T = m + b + x * (h + 1) + 1;
                r[6 * p] = E, r[6 * p + 1] = y, r[6 * p + 2] = T, r[6 * p + 3] = y, r[6 * p + 4] = A, r[6 * p + 5] = T, p++
            }
        }
    }
}
class Box extends Geometry {
    constructor(t, {
        ...
        let o = s,
            ...
            $ = 0,
            f = 0;
        Plane.buildPlane(d, g, p, m, r, i, e, u, l, 2, 1, 0, -1, -1, $, f), Plane.buildPlane(d, g, p, m, r, i, -e, u, l, 2, 1, 0, 1, -1, $ += (u + 1) * (l + 1), f += u * l), Plane.buildPlane(d, g, p, m, e, r, i, u, l, 0, 2, 1, 1, 1, $ += (u + 1) * (l + 1), f += u * l), Plane.buildPlane(d, g, p, m, e, r, -i, u, l, 0, 2, 1, 1, -1, $ += (o + 1) * (u + 1), f += o * u), Plane.buildPlane(d, g, p, m, e, i, -r, o, l, 0, 1, 2, -1, -1, $ += (o + 1) * (u + 1), f += o * u), Plane.buildPlane(d, g, p, m, e, i, r, o, l, 0, 1, 2, 1, -1, $ += (o + 1) * (l + 1), f += o * l), Object.assign(h, {
            position: {
                size: 3,
                data: d
            },
    ...
    constructor(t, {
        ...
        for (let M = 0; M <= u; M++) {
            ...
            for (let S = 0; S <= l; S++, b++) {
                ...
                $[3 * b] = V, $[3 * b + 1] = L, $[3 * b + 2] = N, T.set(V, L, N).normalize(), f[3 * b] = T.x, f[3 * b + 1] = T.y, f[3 * b + 2] = T.z, x[2 * b] = P, x[2 * b + 1] = 1 - F, R.push(w++)
            }
            A.push(R)
        }
        for (let C = 0; C < u; C++)
            for (let O = 0; O < l; O++) {
                let D = A[C][O + 1],
                    ...
                    z = A[C + 1][O],
                    I = A[C + 1][O + 1];
                (0 !== C || d > 0) && (v[3 * E] = D, v[3 * E + 1] = B, v[3 * E + 2] = I, E++), (C !== u - 1 || y < Math.PI) && (v[3 * E] = B, v[3 * E + 1] = z, v[3 * E + 2] = I, E++)
            }
        Object.assign(o, {
            position: {
                size: 3,
    ...
    constructor(t, {
            ...
            for (s = 0; s <= c; s++) {
                ...
                for (t = 0; t <= u; t++) {
                    let g = t / u,
                        ...
                        m = Math.sin(p),
                        y = Math.cos(p);
                    $.set([l * m, (.5 - o) * r, l * y], 3 * b), n.set(m, a, y).normalize(), f.set([n.x, n.y, n.z], 3 * b), x.set([g, 1 - o], 2 * b), h.push(b++)
                }
                E.push(h)
            }
            for (t = 0; t < u; t++)
        ...
        function A(t) {
            let s, n = !0 === t ? e : i,
                a = !0 === t ? 1 : -1,
                h = b;
            for ($.set([0, .5 * r * a, 0], 3 * b), f.set([0, a, 0], 3 * b), x.set([.5, .5], 2 * b), b++, s = 0; s <= u; s++) {
                let o = s / u,
                    l = o * d + _,
                    c = Math.cos(l),
                    g = Math.sin(l);
                $.set([n * g, .5 * r * a, n * c], 3 * b), f.set([0, a, 0], 3 * b), x.set([.5 * c + .5, .5 * g * a + .5], 2 * b), b++
            }
            for (s = 0; s < u; s++) {
                let p = h + s + 1;
                t ? v.set([p, p + 1, h], 3 * w) : v.set([p + 1, p, h], 3 * w), w++
            }
        }
        y(), !a && (e && A(!0), i && A(!1)), Object.assign(l, {
            position: {
                size: 3,
                data: $
            },
...
function Orbit(t, {
    ...
    panSpeed: c = .1,
    minPolarAngle: _ = 0,
    maxPolarAngle: d = Math.PI,
    minAzimuthAngle: g = -1 / 0,
    maxAzimuthAngle: p = 1 / 0,
    minDistance: m = 0,
    maxDistance: $ = 1 / 0
} = {}) {
    this.enabled = i, this.target = r, s = s || 1, n = n || 1, this.minDistance = m, this.maxDistance = $;
    let f = {
            radius: 1,
            phi: 0,
            theta: 0
        ...
        b = new Vec3,
        w = new Vec3;
    w.copy(t.position).sub(this.target), v.radius = x.radius = w.distance(), v.theta = x.theta = Math.atan2(w.x, w.z), v.phi = x.phi = Math.acos(Math.min(Math.max(w.y / x.radius, -1), 1)), this.update = () => {
        x.radius *= f.radius, x.theta += f.theta, x.phi += f.phi, x.theta = Math.max(g, Math.min(p, x.theta)), x.phi = Math.max(_, Math.min(d, x.phi)), x.radius = Math.max(this.minDistance, Math.min(this.maxDistance, x.radius)), v.phi += (x.phi - v.phi) * s, v.theta += (x.theta - v.theta) * s, v.radius += (x.radius - v.radius) * s, this.target.add(b);
        let e = v.radius * Math.sin(Math.max(1e-6, v.phi));
        w.x = e * Math.sin(v.theta), w.y = v.radius * Math.cos(v.phi), w.z = e * Math.cos(v.theta), t.position.copy(this.target).add(w), t.lookAt(this.target), f.theta *= n, f.phi *= n, b.multiply(n), f.radius = 1
    };
    let E = new Vec2,
        y = new Vec2,
        A = new Vec2,
    ...
    let R = (i, r) => {
        ...
        l *= Math.tan((t.fov || 45) / 2 * Math.PI / 180), s = 2 * i * l / o.clientHeight, n = t.matrix, tempVec3$2.set(n[0], n[1], n[2]), tempVec3$2.multiply(-s), b.add(tempVec3$2), a = 2 * r * l / o.clientHeight, h = t.matrix, tempVec3$2.set(h[4], h[5], h[6]), tempVec3$2.multiply(a), b.add(tempVec3$2)
    };
    ...
    function S(t, i) {
        tempVec2a.set(t, i), tempVec2b.sub(tempVec2a, E).multiply(h);
        let r = e === document ? document.body : e;
        f.theta -= 2 * Math.PI * tempVec2b.x / r.clientHeight, f.phi -= 2 * Math.PI * tempVec2b.y / r.clientHeight, E.copy(tempVec2a)
    }

    function P(t, e) {
        tempVec2a.set(t, e), tempVec2b.sub(tempVec2a, y).multiply(c), R(tempVec2b.x, tempVec2b.y), y.copy(tempVec2a)
    }
    let V = t => {
            if (this.enabled) {
                switch (t.button) {
                    case this.mouseButtons.ORBIT:
                        if (!1 === a) return;
                        E.set(t.clientX, t.clientY), T = STATE.ROTATE;
                        break;
                    case this.mouseButtons.ZOOM:
                        if (!1 === o) return;
                        A.set(t.clientX, t.clientY), T = STATE.DOLLY;
                        break;
                    case this.mouseButtons.PAN:
                        if (!1 === u) return;
                        y.set(t.clientX, t.clientY), T = STATE.PAN
                }
                T !== STATE.NONE && (window.addEventListener("mousemove", L, !1), window.addEventListener("mouseup", N, !1))
            }
        },
        L = t => {
            if (this.enabled) switch (T) {
                case STATE.ROTATE:
                    if (!1 === a) return;
                ...
                case STATE.DOLLY:
                    var e;
                    if (!1 === o) return;
                    e = t, tempVec2a.set(e.clientX, e.clientY), tempVec2b.sub(tempVec2a, A), tempVec2b.y > 0 ? F(M()) : tempVec2b.y < 0 && F(1 / M()), A.copy(tempVec2a);
                    break;
                case STATE.PAN:
                    if (!1 === u) return;
        ...
        N = () => {
            this.enabled && (document.removeEventListener("mousemove", L, !1), document.removeEventListener("mouseup", N, !1), T = STATE.NONE)
        },
        C = t => {
            this.enabled && o && (T === STATE.NONE || T === STATE.ROTATE) && (t.stopPropagation(), t.deltaY < 0 ? F(1 / M()) : t.deltaY > 0 && F(M()))
        },
        O = t => {
            if (this.enabled) switch (t.preventDefault(), t.touches.length) {
                case 1:
                    if (!1 === a) return;
                    E.set(t.touches[0].pageX, t.touches[0].pageY), T = STATE.ROTATE;
                    break;
                case 2:
                    if (!1 === o && !1 === u) return;
                    ! function t(e) {
                ...
                case 2:
                    ...
                    ! function t(e) {
                        if (o) {
                            let i = e.touches[0].pageX - e.touches[1].pageX,
                                r = e.touches[0].pageY - e.touches[1].pageY;
                            tempVec2a.set(0, Math.sqrt(i * i + r * r)), tempVec2b.set(0, Math.pow(tempVec2a.y / A.y, l)), F(tempVec2b.y), A.copy(tempVec2a)
                        }
                        if (u) {
                            let s;
    ...
    this.remove = function() {
        e.removeEventListener("contextmenu", z, !1), e.removeEventListener("mousedown", V, !1), window.removeEventListener("wheel", C, !1), e.removeEventListener("touchstart", O, !1), e.removeEventListener("touchend", B, !1), e.removeEventListener("touchmove", D, !1), window.removeEventListener("mousemove", L, !1), window.removeEventListener("mouseup", N, !1)
    }, e.addEventListener("contextmenu", z, !1), e.addEventListener("mousedown", V, !1), window.addEventListener("wheel", C, !1), e.addEventListener("touchstart", O, {
        passive: !1
    }), e.addEventListener("touchend", B, !1), e.addEventListener("touchmove", D, {
        passive: !1
    })
}
...
class Raycast {
    constructor(t) {
        this.gl = t, this.origin = new Vec3, this.direction = new Vec3
    }
    castMouse(t, e = [0, 0]) {
    ...
    intersectBox(t, e = this.origin, i = this.direction) {
        let r, s, n, a, h, o, l = 1 / i.x,
            u = 1 / i.y,
            c = 1 / i.z,
            _ = t.min,
            d = t.max;
        return (r = ((l >= 0 ? _.x : d.x) - e.x) * l, s = ((l >= 0 ? d.x : _.x) - e.x) * l, n = ((u >= 0 ? _.y : d.y) - e.y) * u, r > (a = ((u >= 0 ? d.y : _.y) - e.y) * u) || n > s) ? 0 : (n > r && (r = n), a < s && (s = a), h = ((c >= 0 ? _.z : d.z) - e.z) * c, r > (o = ((c >= 0 ? d.z : _.z) - e.z) * c) || h > s) ? 0 : (h > r && (r = h), o < s && (s = o), s < 0) ? 0 : r >= 0 ? r : s
    }
}
class Post {
    constructor(t, {
    ...
    addPass({
        vertex: t = defaultVertex,
        fragment: e = defaultFragment,
        uniforms: i = {},
    ...
    resize({
        ...
        i && (this.dpr = i), t && (this.width = t, this.height = e || t), i = this.dpr || this.gl.renderer.dpr, t = (this.width || this.gl.renderer.width) * i, e = (this.height || this.gl.renderer.height) * i, this.options.width = t, this.options.height = e, this.fbo.read = new RenderTarget(this.gl, this.options), this.fbo.write = new RenderTarget(this.gl, this.options)
    }
    render({
        scene: t,
        camera: e,
...
class Animation {
    ...
    update(t = 1, e) {
        ...
        this.objects.forEach((t, e) => {
            prevPos.fromArray(a.position, 3 * e), prevRot.fromArray(a.quaternion, 4 * e), prevScl.fromArray(a.scale, 3 * e), nextPos.fromArray(h.position, 3 * e), nextRot.fromArray(h.quaternion, 4 * e), nextScl.fromArray(h.scale, 3 * e), prevPos.lerp(nextPos, n), prevRot.slerp(nextRot, n), prevScl.lerp(nextScl, n), t.position.lerp(prevPos, i), t.quaternion.slerp(prevRot, i), t.scale.lerp(prevScl, i)
        })
    }
}
...
class Skin extends Mesh {
    ...
    createBones(t) {
        if (this.root = new Transform, this.bones = [], t.bones && t.bones.length) {
    ...
    createBoneTexture() {
        ...
        this.boneMatrices = new Float32Array(t * t * 4), this.boneTextureSize = t, this.boneTexture = new Texture(this.gl, {
    ...
    draw({
        ...
        this.root.updateMatrixWorld(!0), this.bones.forEach((t, e) => {
            tempMat4$2.multiply(t.worldMatrix, t.bindInverse), this.boneMatrices.set(tempMat4$2, 16 * e)
...
function Text({
    ...
    function $() {
        _ = t.common.lineHeight, g = s / (d = t.common.base);
        let i = e.replace(/[ \n]/g, "").length;
        c = {
        ...
        for (let r = 0; r < i; r++) c.id[r] = r, c.index.set([4 * r, 4 * r + 2, 4 * r + 1, 4 * r + 1, 4 * r + 2, 4 * r + 3], 6 * r);
        f()
    }
    ...
    function f() {
        ...
        for (; d < e.length && w < 100;) {
            ...
            let M = 0;
            if (m.test(E) ? ($ = d, f = 0, M += h * s) : M += n * s, M += y.xadvance * g, v.width += M, f += M, v.width > i) {
                if (o && v.glyphs.length > 1) {
                    v.width -= M, v.glyphs.pop(), v = b();
                    continue
                }
                if (!o && f !== v.width) {
                    let R = d - $ + 1;
                    v.glyphs.splice(-R, R), d = $, v.width -= f, v = b();
                    continue
                }
            }
            d++
            ...
            function e(i) {
                ...
                for (let _ = 0; _ < i.length; _++) {
                    ...
                    for (let p = 0; p < d.glyphs.length; p++) {
                        let $ = d.glyphs[p][0],
                            f = d.glyphs[p][1];
                        if ("center" === r ? f -= .5 * d.width : "right" === r && (f -= d.width), m.test($.char)) continue;
                        f += $.xoffset * g, o -= $.yoffset * g;
                        let x = $.width * g,
                            v = $.height * g;
                        ...
                        let b = $.x / n,
                            ...
                            E = 1 - $.y / h,
                            y = $.height / h;
                        c.uv.set([b, E - y, b, E, b + w, E - y, b + w, E, ], 8 * u), o += $.yoffset * g, u++
                    }
                    o -= s * a
                }
                l.buffers = c, l.numLines = i.length, l.height = l.numLines * s * a
            }(_)
    }
    ...
    function x(e, i) {
        for (let r = 0; r < t.kernings.length; r++) {
            ...
            if (!(s.first < e) && !(s.second < i)) {
                if (s.first > e || s.first === e && s.second > i) return 0;
                return s.amount
            }
        }
        return 0
    }
    u = {}, t.chars.forEach(t => u[t.char] = t), $(), this.resize = function(t) {
        ({
            width: i
        } = t), f()
...
class Flowmap {
    constructor(t, {
        ...
        function a() {
            ...
            n.mask.read = new RenderTarget(t, r), n.mask.write = new RenderTarget(t, r), n.mask.swap()
        }
        ...
        }, a(), this.aspect = 1, this.mouse = new Vec2, this.velocity = new Vec2, this.mesh = h()
    }
    update() {
...
class GPGPU {
    ...
    addPass({
        vertex: t = defaultVertex$1,
        fragment: e = defaultFragment$1,
        uniforms: i = {},
        textureUniform: r = "tMap",
        enabled: s = !0
    } = {}) {
...
class Polyline {
    constructor(t, {
        points: e,
        vertex: i = defaultVertex$2,
        fragment: r = defaultFragment$2,
        uniforms: s = {},
        attributes: n = {}
    }) {
        ...
        for (let l = 0; l < this.count; l++) {
            ...
            if (h.set([0, u, 1, u], 4 * l), l === this.count - 1) continue;
            let c = 2 * l;
            o.set([c + 0, c + 1, c + 2], (c + 0) * 3), o.set([c + 2, c + 1, c + 3], (c + 1) * 3)
        }
        ...
        this.updateGeometry(), s.uResolution || (this.resolution = s.uResolution = {
            value: new Vec2
    ...
    updateGeometry() {
        this.points.forEach((t, e) => {
            t.toArray(this.position, 6 * e), t.toArray(this.position, 6 * e + 3), e ? (t.toArray(this.next, (e - 1) * 6), t.toArray(this.next, (e - 1) * 6 + 3)) : (tmp.copy(t).sub(this.points[e + 1]).add(t), tmp.toArray(this.prev, 6 * e), tmp.toArray(this.prev, 6 * e + 3)), e === this.points.length - 1 ? (tmp.copy(t).sub(this.points[e - 1]).add(t), tmp.toArray(this.next, 6 * e), tmp.toArray(this.next, 6 * e + 3)) : (t.toArray(this.prev, (e + 1) * 6), t.toArray(this.prev, (e + 1) * 6 + 3))
...
class Shadow {
    constructor(t, {
        light: e = new Camera(t),
        width: i = 1024,
        height: r = i
    }) {
        this.gl = t, this.light = e, this.target = new RenderTarget(t, {
            width: i,
            height: r
    ...
    add({
        ...
        receive: e = !0,
        cast: i = !0,
        vertex: r = defaultVertex$3,
        fragment: s = defaultFragment$3,
        uniformProjection: n = "shadowProjectionMatrix",
        uniformView: a = "shadowViewMatrix",
        uniformTexture: h = "tShadow"
    }) {
        if (e && !t.program.uniforms[n] && (t.program.uniforms[n] = {
            ...
            }), i && (this.castMeshes.push(t), t.colorProgram = t.program, !t.depthProgram)) {
    ...
    render({
        ...
        t.traverse(t => {
            t.draw && (~this.castMeshes.indexOf(t) ? t.program = t.depthProgram : (t.visible && (t.isForceVisibility = !0), t.visible = !1))
        }), this.gl.renderer.render({
            scene: t,
            camera: this.light,
            target: this.target
